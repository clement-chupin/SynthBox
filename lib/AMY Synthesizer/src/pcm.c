...
void pcm_note_on(uint16_t osc) {
    if(AMY_IS_SET(synth[osc]->preset)) {
        ...
        // Respect trigger_phase if set (e.g. FUL circular start at slice N).
        // amy.c already applied F2P(trigger_phase) → phase before calling osc_note_on.
        // If trigger_phase was not set, reset to 0 as usual.
        if (!AMY_IS_SET(synth[osc]->trigger_phase)) {
            synth[osc]->phase = 0; // s16.15 index into the table
        }
        AMY_UNSET(synth[osc]->trigger_phase); // consume so future note-ons reset to 0
    ...
    if(AMY_IS_SET(synth[osc]->preset)) {
        ...
        for(uint16_t i=0; i < AMY_BLOCK_SIZE; i++) {
            ...
            if (preset->channels == 2) {
                ...
                if (preset->bit_depth == 8) {
                    const int8_t* t8 = (const int8_t*)table;
                    b = (LUTSAMPLE)((int32_t)t8[base_index] << 8);
                    c = (next_index < sample_length) ? (LUTSAMPLE)((int32_t)t8[next_index] << 8) : b;
                } else {
                    b = table[base_index];
...
int pcm_load_file() {
    ...
    // Fully initialise the node before making it visible in the LL so that the
    // render task (running concurrently on Core 0) never sees a partially-ready node.
    ...
    // Atomic lock-free prepend: retries if another core concurrently modified the head.
    do { new_preset_pointer->next = memorypcm_ll_start; }
    while (!__sync_bool_compare_and_swap(&memorypcm_ll_start,
                                         new_preset_pointer->next, new_preset_pointer));
...
int16_t * pcm_load(uint16_t preset_number, uint32_t length, uint32_t samplerate, uint8_t channels, uint8_t midinote, uint32_t loopstart, uint32_t loopend) {
    size_t need = sizeof(memorypcm_ll_t) + sizeof(memorypcm_preset_t) + length * channels * sizeof(int16_t);
    // Try the new allocation BEFORE touching the old preset (if any) for this slot, so
    // a failed/reduced load leaves the previous sample intact and playable instead of
    // destroying it first and then finding out there's no room for the replacement.
    // Only fall back to freeing the old one first (old sound is lost, as before) if the
    // extra headroom it frees is actually needed for the new allocation to succeed.
    memorypcm_ll_t *new_preset_pointer = malloc_caps(need, amy_global.config.ram_caps_sample);
    if (new_preset_pointer == NULL) {
        pcm_unload_preset(preset_number);
        new_preset_pointer = malloc_caps(need, amy_global.config.ram_caps_sample);
        if (new_preset_pointer == NULL) {
            fprintf(stderr, "No RAM left for sample load\n");
...
int8_t* pcm_load8(uint16_t preset_number, uint32_t length, uint32_t samplerate, uint8_t channels, uint8_t midinote, uint32_t loopstart, uint32_t loopend) {
    size_t need = sizeof(memorypcm_ll_t) + sizeof(memorypcm_preset_t) + length * channels * sizeof(int8_t);
    // Same "try before freeing the old one" order as pcm_load() above — see its comment.
    memorypcm_ll_t *new_preset_pointer = malloc_caps(need, amy_global.config.ram_caps_sample);
    if (new_preset_pointer == NULL) {
        pcm_unload_preset(preset_number);
        new_preset_pointer = malloc_caps(need, amy_global.config.ram_caps_sample);
    ...
    while (!__sync_bool_compare_and_swap(&memorypcm_ll_start,
                                         new_preset_pointer->next, new_preset_pointer));
    return (int8_t*)memory_preset->sample_ram;
}
...
// Register a preset that points into an externally-owned buffer (e.g. memory-mapped flash).
// Allocates only the LL node + preset struct — sample data is NOT owned by this alloc.
// pcm_unload_preset frees only the node, which is correct: the flash region stays mapped.
void pcm_register_extern8(uint16_t preset_number, const int8_t* data, uint32_t length,
                           uint32_t samplerate, uint8_t midinote,
                           uint32_t loopstart, uint32_t loopend) {
    pcm_unload_preset(preset_number);
    memorypcm_ll_t *p = malloc_caps(sizeof(memorypcm_ll_t) + sizeof(memorypcm_preset_t),
                                     amy_global.config.ram_caps_sample);
    if (p == NULL) { fprintf(stderr, "pcm_register_extern8: alloc fail\n"); return; }
    p->preset_number = preset_number;
    memorypcm_preset_t *m = (memorypcm_preset_t*)(((uint8_t*)p) + sizeof(memorypcm_ll_t));
    m->samplerate           = samplerate;
    m->log2sr               = log2f((float)samplerate / ZERO_LOGFREQ_IN_HZ);
    m->midinote             = midinote;
    ...
    m->bit_depth            = 8;
    m->filename[0]          = '\0';
    m->file_bytes_remaining = 0;
    m->file_handle          = 0;
    m->type                 = AMY_PCM_TYPE_MEMORY;
    m->sample_ram           = (int16_t*)data;  // points into mmap'd flash — not part of this alloc
    m->loopend              = (loopend == 0) ? length - 1 : loopend;
    p->preset = m;
    do { p->next = memorypcm_ll_start; }
    while (!__sync_bool_compare_and_swap(&memorypcm_ll_start, p->next, p));
}

// Register an external 16-bit buffer as a PCM preset without copying the data.
// The caller owns the buffer and must keep it alive while the preset is in use.
void pcm_register_extern16(uint16_t preset_number, const int16_t* data, uint32_t length,
                            uint32_t samplerate, uint8_t midinote,
                            uint32_t loopstart, uint32_t loopend) {
    uint32_t le = (loopend == 0) ? length - 1 : loopend;
    // If the preset already exists as a memory preset, update its fields in-place.
    // This avoids the unload→reload gap during which the audio thread falls back to a ROM preset,
    // potentially triggering spurious SYNTH_OFF (base_index >= ROM length) on active LOP oscillators.
    memorypcm_ll_t *node = memorypcm_ll_start;
    while (node != NULL) {
    ...
    // Preset not found: unload any stale entry (file-type, etc.) and allocate a fresh node.
    pcm_unload_preset(preset_number);
    memorypcm_ll_t *p = malloc_caps(sizeof(memorypcm_ll_t) + sizeof(memorypcm_preset_t),
                                     amy_global.config.ram_caps_sample);
    if (p == NULL) { fprintf(stderr, "pcm_register_extern16: alloc fail\n"); return; }
    p->preset_number = preset_number;
    memorypcm_preset_t *m = (memorypcm_preset_t*)(((uint8_t*)p) + sizeof(memorypcm_ll_t));
    m->samplerate           = samplerate;
    m->log2sr               = log2f((float)samplerate / ZERO_LOGFREQ_IN_HZ);
